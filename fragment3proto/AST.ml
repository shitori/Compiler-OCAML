...
and cas_switch_a =
      Cswt  of expression_a * programme_a * cas_switch_a * int
and expression_a =
    ...
    | CallV of string * int
    | CallO of expression_a * string * int
    | Eobj  of cont_objet_a * int
    | Nul   of int
    | Lmbd  of arguments_a * expression_a * int
and cont_objet_a =
      Obj   of string * expressions_a * int
    | Objs  of string * expressions_a * cont_objet_a * int
and affect_expr_a =
...
and commande_a =
    ...
    | Swt   of expression_a * cas_switch_a * int
    | SwtV  of expression_a * int
and programme_a =
      Nill of int
...
and getTailleCO cont_objet = match cont_objet with
        | Obj   (i,es,t)    -> t
        | Objs  (i,es,co,t) -> t

and getTailleCS cas_switch = match cas_switch with
        | Cswt  (e,p,cs,t)  -> t
;;
...
and print_AST_exp form = let open Format in function
    ...
    | CallO (e,i,t)         -> fprintf form "@[<2>%i%s@ %s%s@ %a%s@]"               t "CallObj(" i "," print_AST_exp e ")"
    | Eobj  (o,t)           -> fprintf form "@[<2>%i%s@ %a%s@]"                     t "EObj(" print_AST_co o ")"
    | Nul   t               -> fprintf form "@[<2>%i%s@]"                           t "Nul()"
    | Lmbd  (a,e,t)         -> fprintf form "@[<2>%i%s@ %a%s@ %a%s@]"               t "Lambda(" print_AST_arg a " ," print_AST_exp e ")"
and print_AST_com form = let open Format in function
    ...
    | Swt   (e,cs,t)        -> fprintf form "@[<2>%i%s@ %a%s@ %a%s@]"               t "Switch(" print_AST_exp e " ," print_AST_cs cs ")"
    | SwtV  (e,t)           -> fprintf form "@[<2>%i%s@ %a%s@]"                     t "SwitchV(" print_AST_exp e ")"
...
and print_AST_co form = let open Format in function
    | Obj   (i,es,t)        -> fprintf form "@[<2>%i%s@ %s%s@ %a%s@]"               t "Obj(" i " ," print_AST_exps es ")"
    | Objs  (i,es,co,t)     -> fprintf form "@[<2>%i%s@ %s%s@ %a%s@ %a%s@]"         t "Objs(" i "," print_AST_exps es " ," print_AST_co co ")"
and print_AST_cs form = let open Format in function
    | Cswt  (e,p,cs,t)      -> fprintf form "@[<2>%i%s@ %a%s@ %a%s@ %a%s@]"         t "CasSwitch(" print_AST_exp e " ," print_AST_prog p " ," print_AST_cs cs ")"
;;
...
and print_com = function
    ...
    | Tq    (e,c,t)         -> (print_exp e)^"\nConJmp "^string_of_int ((getTailleCommande c)+1)^"\n"^(print_com c)^"\nJump "^string_of_int (((getTailleCommande c)+(getTailleExpression e)+2)*(-1))
...
and taille_exp w = match w with
         ...
         | CallV   (i,t)        -> t
         | CallO (e,i,t)        -> 0 + (taille_exp e)
         | Eobj  (o,t)          -> 0 + (taille_co o)
         | Nul   t              -> t
         | Lmbd  (a,e,t)        -> 0 + (taille_exp e)
 ...
 and taille_com c = match c with
         ...
         | Swt   (e,cs,t)           -> 0 + (taille_exp e) + (taille_cs cs)
         | SwtV  (e,t)              -> 0 + (taille_exp e)
 ...
 and taille_co co = match co with
         | Obj   (i,es,t)    -> 0 + (taille_exps es)
         | Objs  (i,es,co,t) -> 0 + (taille_exps es) + (taille_co co)

 and taille_cs cs = match cs with
         | Cswt  (e,p,cs,t)  -> 0 + (taille_exp e) + (taille_prog p) + (taille_cs cs)
